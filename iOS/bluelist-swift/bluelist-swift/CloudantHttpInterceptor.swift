...
class CloudantHttpInterceptor: NSObject, CDTHTTPInterceptor {
    ...
    func obtainSessionCookie(completionHandler:(error:NSError?)->Void){
        WLAuthorizationManager.sharedInstance().obtainAuthorizationHeaderForScope("cloudant") { (response, error) -> Void in
            if(error != nil){
                completionHandler(error: error)
            }else{
                let request:WLResourceRequest = WLResourceRequest(URL: self.refreshSessionCookieUrl, method: "POST")
